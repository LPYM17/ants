...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if there is bee, return that bee, if not found then return None 
...
Ensure that Bees in the Hive are never returned by nearest_bee.
        """
        # BEGIN Problem 3 and 4, loop from ant current position to the end, 
        current_place = self.place
        range = 0
        while current_place != None and current_place.is_hive != True:
            if range<= self.upper_bound and range >= self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        if self.place: 
            bees_in_area = list(self.place.bees)
            for bee in bees_in_area:
                bee.reduce_health(amount)
        
            ##if die then we need to damage all bees in the area 
        if self.health - amount <=0 and self.place:
            bees_in_area = list(self.place.bees)
            for bee in bees_in_area:
    ...
    def action(self, gamestate):
        if self.cooldown > 0:
            ...
            if self.place and self.place.bees: 
                chosen_bee_to_eat = random_bee(self.place.bees)
                if chosen_bee_to_eat: 
                    chosen_bee_to_eat.reduce_health(chosen_bee_to_eat.health) 
...
class ProtectorAnt(ContainerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 8c
    implemented = True   # Change to True to view in the GUI hello
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        #water inhereit from place, add the insect to the water, if proof then ok, 
        # if not water proof then minus 1 health = 0 
        # END Problem 10
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current_place is not None: 
            ...
            if ant:
                if ant.is_container and ant.ant_contained and not ant.ant_contained.doubled:
